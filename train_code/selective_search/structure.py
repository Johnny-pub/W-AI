...
from train_code.selective_search import measure11
...
class HierarchicalGrouping(object):
    ...
    def build_regions(self):
        self.regions = {}
        lbp_img = measure11.generate_lbp_image(self.img)
        for label in self.labels:
            ...
            color_hist = measure11.calculate_color_hist(mask, self.img)
            texture_hist = measure11.calculate_texture_hist(mask, lbp_img)
    ...
    def build_region_pairs(self):
        ...
        for i in self.labels:
            ...
            for j in neighbors:
                if i < j:
                    self.s[(i, j)] = measure11.calculate_sim(self.regions[i],
                                                             self.regions[j],
                                                             self.img.size,
                                                             self.sim_strategy)
    ...
    def calculate_similarity_for_new_region(self):
        ...
        for j in neighbors:
            ...
            self.s[(j, i)] = measure11.calculate_sim(self.regions[i],
                                                     self.regions[j],
                                                     self.img.size,
                                                     self.sim_strategy)
